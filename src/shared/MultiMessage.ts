...
const tempEmbed = new EmbedBuilder().setColor(0xffff00).setTitle('...')

const errorEmbed = new EmbedBuilder()
  .setColor(0xff0000)
  .setTitle(
    'Error... You can ask GPTChat to repeat itself if it cut off too early.'
  )
        ...
        logMessage(
          `Cannot send a message: [${
            this.channel.isDMBased()
              ? 'DM'
              : this.channel.guild?.name ?? this.channel.guildId
          }] <#${this.channel.id}>`
        )
      } else {
        console.error(
    ...
    for (let i = 0; i < sections.length; ++i) {
      ...
      if (this.messageList.length > i) {
        ...
        if (messageListElement.cached === section) {
          ...
          await messageListElement.message.edit({
            content: section,
            embeds: [],
          })
        } else {
        ...
        if (this.messageToReplyTo != null && this.messageList.length === 0) {
          newMessage = await this.messageToReplyTo.reply({
            content: section,
            embeds,
          })
        } else {
