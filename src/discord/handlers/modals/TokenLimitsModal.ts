...
export const TokenLimitsModal = defineModal(
  ...
  async interaction => {
    ...
    return {
      ...
      maxAllowedTokens: `${
        config.modelInfo['gpt-3.5-turbo'].MAX_ALLOWED_TOKENS || 0
      }`,
    }
  },
      ...
      if (
        ...
        await submitInteraction.followUp({
          ...
          embeds: [
            new EmbedBuilder()
              .setDescription(
                `Updated token limits to [${maxTokensForRecentMessagesValue}, ${maxAllowedTokensValue}]!`
              )
              .setColor(0x00ff00),
          ],
          components: [],
