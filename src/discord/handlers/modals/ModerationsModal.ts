...
import {
  getConfig,
  getDefaultModerationThresholds,
  setConfig,
} from '../../../core/config'

export function moderationResultToString(
  moderationThreshold: CreateModerationResponseResultsInnerCategoryScores
) {
  return JSON.stringify(
    Object.fromEntries(
      Object.entries(moderationThreshold).map(([key, value]) => [
        key,
        value.toFixed(2),
      ])
    ),
...
export const ModerationsModal = defineModal(
  ...
  async (values, submitInteraction) => {
    try {
      ...
      const fromJSON: CreateModerationResponseResultsInnerCategoryScores =
        JSON.parse(moderationThresholds!)

      Object.entries(config.moderationThreshold).map(([key]) => {
        const keyToUse =
        ...
        if (isNaN(newValue) || newValue < 0 || newValue > 1) {
          throw new Error(
            `Cannot set ${key}: invalid value ${newValue}, needs to be a number between 0 and 1`
          )
        }
...
`
      ...
      await submitInteraction.followUp({
        ...
        embeds: e.message
          ? [new EmbedBuilder().setDescription(e.message).setColor(0xff0000)]
          : [],
        components: [],
      })
    }
