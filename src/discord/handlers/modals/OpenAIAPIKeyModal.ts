...
export const OpenAIAPIKeyModal = defineModal(
  ...
  async (values, submitInteraction) => {
    try {
      ...
      const { configId, isDM } =
        await getConfigIdForInteraction(submitInteraction)
      ...
      if (apiKey == null || apiKey.length === 0) {
        ...
        await submitInteraction.followUp({
          ...
          embeds: [
            new EmbedBuilder()
              .setDescription(
                `Removed OpenAI API key for [${
                  isDM
                    ? `User:${submitInteraction.user.tag}`
