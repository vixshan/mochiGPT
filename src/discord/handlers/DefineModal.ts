...
export const defineModal = <
  TInputs extends ModalInput[],
  TButtonConfig extends ModalButtonConfig | null,
>(
  id: string,
  title: string,
  ...
  getCurrentValues: (
    interaction: MessageComponentInteraction | CommandInteraction
  ) => Promise<ModalValues<TInputs>>,
  onSubmit: (
    values: ModalValues<TInputs>,
  ...
  const show = async (
    ...
    logMessage(
      `Showing modal \`${id}\`: \`${title}\` for ${interaction.user.tag}.`
    )
    ...
    try {
      ...
      modal.addComponents(
        inputs
          .filter(input => {
            const shouldShow = input.shouldShow as (
              interaction: MessageComponentInteraction | CommandInteraction
            ) => boolean
      ...
      return await interaction.showModal(modal).then(
        () => true,
        e => {
  ...
  const result: ModalConfig<false> = {
    ...
    async run(client, submitInteraction) {
      ...
      logMessage(
        `Submitted modal ${id}-${title} from ${submitInteraction.user.tag}: `,
        allFields
      )
      ...
      inputs.forEach(input => {
        if (allFields.get(input.name)) {
          values[input.name as keyof ModalValues<TInputs>] =
            submitInteraction.fields.getTextInputValue(input.name)
        } else {
          values[input.name as keyof ModalValues<TInputs>] = ''
        }
      })
