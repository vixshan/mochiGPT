...
async function handleChat(
  ...
  if (!openAI) {
    ...
    if (!openAI) {
      logMessage(
        `Could not find API key for server ${await getGuildName(
          interaction.guildId
        )}[${interaction.guildId}}] or user [[${interaction.user.username}|${
          interaction.user.id
        }]]`
      )
  ...
  const firstMessage = `${
    value ? `<@${userId}>: ${value}` : `Chat with <@${userId}>`
  }`
  ...
  if (isPrivate) {
    message = await interaction.followUp({
      ephemeral: true,
      embeds: [
        new EmbedBuilder().setDescription('Creating new private thread...'),
      ],
    })
    ...
    referenceThreadHere = await interaction.followUp({
      ...
      embeds: [
        new EmbedBuilder().setDescription('Creating new thread in channel...'),
      ],
    })
  ...
  try {
    ...
    if (isPrivate) {
      const messageChannel = await discordClient.channels.fetch(
        message.channelId
      )
    ...
    try {
      await interaction.followUp(
        'Could not create thread... Please ask an admin for permissions!'
      )
    } catch (e) {
      logMessage(
        `Cannot even follow up in <#${message.channelId}> of ${message.guild?.name}.`
      )
    }
    logMessage(
      `Cannot create thread in <#${message.channelId}> of ${message.guild?.name}.`,
      e
    )
    return
  }
  ...
  const existingConvo = (await retrieveConversation(
    interaction.channelId
  )) as ChatGPTConversation | null

  if (
    existingConvo &&
    existingConvo.version == ChatGPTConversation.latestVersion
  ) {
  ...
  logMessage(
    `New thread by <@${user.id}> in ${await conversation.getLinkableId()}.`
  )
