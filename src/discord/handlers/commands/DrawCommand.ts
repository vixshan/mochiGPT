      ...
      run: async (client: Client, interaction: CommandInteraction) => {
        ...
        try {
          if (channel.isTextBased() && !channel.isDMBased()) {
            const guildMember = await channel.guild.members.fetch(
              client.user!.id
            )

            const permissions = channel.permissionsFor(guildMember.id)!
          ...
          logMessage(
            `No api key for [${
              isDirectMessage
                ...
                : await getGuildName(interaction.guildId)
            }].`
          )
        ...
        if (!isDirectMessage) {
          ...
          const userHasExceptionRole = currentConfig.exceptionRoleIds.some(
            exceptionRoleId => member.roles.cache.has(exceptionRoleId)
          )
          ...
          if (userShouldBeRestricted) {
            const messageCountForUser: MessageCountInfo =
              getMessageCountForUser(messageCounter, userId)

            if (
              messageCountForUser.imageLimitCount >=
              currentConfig.maxImagePerUser
            ) {
              await interaction.followUp({
                ephemeral: true,
                content: '',
