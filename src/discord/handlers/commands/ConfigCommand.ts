...
import {
  moderationResultToString,
  ModerationsModal,
} from '../modals/ModerationsModal'
...
export async function getConfigIdForInteraction(
...
async function generateFollowUp(configId: string, isDM: boolean, user: User) {
  ...
  const fields = [
    {
      name: 'Token limits:',
      value: `Max tokens for prompt: ${
        config.modelInfo['gpt-3.5-turbo'].MAX_ALLOWED_TOKENS
      }.
...
With this configuration, each message can cost at most \$${(
        (0.02 * config.modelInfo['gpt-3.5-turbo'].MAX_ALLOWED_TOKENS) /
        1000
      ).toFixed(2)} USD.
...
If max tokens for recent messages are less than max tokens for prompt, then the rest of the tokens will be used for the longer term memory.`,
  ...
  if (USE_SAME_API_KEY_FOR_ALL !== 'true' || configId === mainServerId) {
    fields.push({
      ...
      value: `${
        config.openAIApiKey
          ? `✅ ${config.openAIApiKey.slice(0, 3)}${config.openAIApiKey
              .slice(3)
              .replace(/./g, 'x')}`
          : '❌ Missing!'
      }
...
You can find your API key at [https://beta.openai.com/account/api-keys](https://beta.openai.com/account/api-keys).`,
  ...
  if (!isDM) {
    ...
    const totalSum = Object.values(messageCounter).reduce(
...
Total messages sent by all users of this server's API key: ${totalSum}.`,
      ...
      const messageCountForUser = getMessageCountForUser(
        messageCounter,
        user.id
      )
      ...
      fields.push({
        name: 'Sent Messages',
        value: `You sent ${messageCountForUser.limitCount}/${
          config.maxMessagePerUser === -1
            ? 'Unlimited'
            : config.maxMessagePerUser
        } messages.

You generated ${messageCountForUser.imageLimitCount}/${
          config.maxImagePerUser === -1 ? 'Unlimited' : config.maxImagePerUser
        } images.
...
Total messages sent by all users of this server's API key: ${totalSum}.`,
    ...
    fields.push({
      ...
      value: config.useKeyInServersToo
        ? 'Your API key will be used in all servers.'
        : 'Your API key will only be used in DMs.',
    })
    ...
    components[0] = components[0].addComponents(
      new ButtonBuilder()
        ...
        .setLabel(
          config.useKeyInServersToo
            ? 'Use my API key for DMs only'
            : 'Use my API key for server conversations'
        )
        .setStyle(
          config.useKeyInServersToo ? ButtonStyle.Danger : ButtonStyle.Primary
        )
    )
  }
  ...
  const embedForServerOrUser = new EmbedBuilder()
    .setTitle(
      `${
        isDM ? `USER [${user.tag}]` : `SERVER [${await getGuildName(configId)}]`
      } Config`
    )
    .setFields(fields)

  const embeds = [embedForServerOrUser]
  ...
  if (!isDM) {
    embeds.push(
      new EmbedBuilder()
        ...
        .setDescription(
          `${
            config.chatChannelIds.length > 0
  ...
  run: async (client: Client, commandInteraction: CommandInteraction) => {
    ...
    if (!channel) {
      await commandInteraction.followUp({
        content: 'Interaction somehow not in a channel?',
      })
      return
    }
    ...
    const { configId, isDM } =
      await getConfigIdForInteraction(commandInteraction)

    if (
      commandInteraction.user.id === adminPingId &&
      !isDM &&
      configId === mainServerId
    ) {
...
As new messages are sent, they will be stored in long term memory one by one, so this value is useful only for conversations before V2. If you don't care about old conversations, set this to be 0.`,
      ...
      if (CUSTOM_PROMPT_COMMAND_NAME) {
        fields.push({
          ...
          value: `${(
            await Promise.all(
              config.promptPermissions.map(async item => {
                try {
...
If you'd like to use the server's API key, please send me the /${CONFIG_COMMAND_NAME} command in a DM.`,
            ...
            await commandInteraction.followUp({
              ephemeral: true,
              embeds: [new EmbedBuilder().setFields(fields)],
            })
          ...
          if (configId != null) {
            ...
            logMessage(
              `Showing config for [${
                isDM
                  ? `User:${commandInteraction.user.tag}`
                  : `Server:${await getGuildName(configId)}, by User:${
                      commandInteraction.user.tag
                    }`
              }]`
            )
            ...
            const fields = [
              ...
              },
              {
                ...
                value: `You generated ${messageCountForUser.imageLimitCount}/${
                  config.maxImagePerUser === -1
                    ? 'Unlimited'
                    : config.maxImagePerUser
                } images.`,
              },
            ]
