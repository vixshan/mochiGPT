...
async function handleModalSubmit(
  ...
  const modalSubmit = ModalSubmitHandlers.find(
  ...
  try {
    ...
    logMessage(
      `Cannot run modal submit command in guild [${
...
const handleButtonCommand = async (
  ...
  const modalButtonCommand = ModalSubmitHandlers.find(
    c => c.hasButton && c.buttonId === interaction.customId
  ) as ModalConfig<true>
  if (modalButtonCommand) {
    try {
      ...
      logMessage(
        `Cannot run button command in guild [${
          interaction.guild?.name ?? 'Unknown Guild'
        }]`,
        e
      )
