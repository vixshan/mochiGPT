...
import {
  getLastMessagesUntilMaxTokens,
  getNumTokens,
} from './GetLastMessagesUntilMaxTokens'
...
export const messageToPromptPart = (item: MessageHistoryItem): string => {
  ...
  const timeInfo = item.timestamp
    ? `[${messageFormattedDateTime(new Date(item.timestamp))}] `
    : ''

  return `${timeInfo}${
    item.username
  }:${endOfPrompt} ${item.content.trimStart()}`
}
...
export function ignoreInput(inputValue: string) {
  return (
    inputValue.match(/^\s*~/) || inputValue.match(/^\s*\[\[\s*ps+t\s*\]\]/i)
  )
}
...
export class ChatGPTConversation extends BaseConversation {
  ...
  public static async handleRetrievalFromDB(fromDb: ChatGPTConversation) {
    ...
    if (fromDb.version === 2) {
      logMessage(
        `trying to upgrade from v2 ${await BaseConversation.GetLinkableId(
          fromDb
        )}(${fromDb.threadId})!`
      )
    ...
    if (versionUpgraded) {
      logMessage(
        `upgraded from v2 ${await BaseConversation.GetLinkableId(fromDb)}(${
          fromDb.threadId
        })!`
      )
  ...
  async createResponseMessage(
    ...
    const embeddingId = await this.tryCreateEmbeddingForMessage(
      openai,
      user,
      message,
      timestamp,
  ...
  private async tryCreateEmbeddingForMessage(
    openai: OpenAIApi,
    user: User,
    message: string,
    timestamp: number,
    ...
    if (pinecone) {
      ...
      try {
        ...
        if (e.isAxiosError) {
          // response = e.response;
          logMessage(
            `Could not create embedding... ${await this.getLinkableId()}`,
            e.response.data
          )
        } else {
          const adminPingId = getEnv('ADMIN_PING_ID')
  ...
  private async SendPromptToGPTChat(
    ...
    const inputMessageItem = await this.createHumanMessage(
      openai,
      user,
      message
    )
    ...
    try {
      fullPrompt = await this.getFullPrompt(
        config,
        openai,
        user,
        inputMessageItem,
        relevancyResultsCache
      )
    } catch (e) {
      onProgress(
        '[[Could not create a prompt :( Please message the bot owner for more info.]]',
        true
      )
    ...
    if (typeof response !== 'string') {
      switch (response.type) {
        case CompletionErrorType.Unknown:
          onProgress(
            '[[Unknown error from OpenAI servers. Please ping the bot owner for help.]]',
            true
          )
          break
        case CompletionErrorType.NoResponse:
          onProgress(
            '[[Could not get a response from OpenAI :( Perhaps their servers are down?]]',
            true
          )
          break
        case CompletionErrorType.OutOfTokens:
          onProgress(
            `[[Whoops, ran out of tokens :( Contact your OpenAI account holder please.${
              usingOpenAIForServer
                ? `

You can alternatively supply your own API key to me by sending me the /${CONFIG_COMMAND_NAME} command in a DM.`
                : ''
            }]]`,
            true
          )
    ...
    const createResponseMessagePromise = this.createResponseMessage(
      openai,
      this.username,
      user,
      response,
  ...
  async handlePrompt(
    ...
    if (
      ...
      if (channel.type === ChannelType.GuildText) {
        const threadName = `${user.username} - ${
          inputValue ?? getDateString(new Date())
        }`.substring(0, 80)
    ...
    logMessage(
      `PROMPT: [${
        user.username
      }] in ${await this.getLinkableId()}: ${inputValue}`
    )
    ...
    if (usingOpenAIForServer && currentConfig.maxMessagePerUser !== -1) {
      ...
      const messageCountForUser: MessageCountInfo = getMessageCountForUser(
        messageCounter,
        userId
      )
      ...
      if (messageCountForUser.limitCount > currentConfig.maxMessagePerUser) {
        ...
        if (
          !currentConfig.exceptionRoleIds.some(exceptionRoleId =>
            member.roles.cache.has(exceptionRoleId)
          )
        ) {
          await trySendingMessage(
            channel,
            {
          ...
          const guildInfo = await Promise.all(
            guilds.map(async guild => {
              try {
                ...
                const threads: KeyValuePair<ChatGPTConversation>[] = await db
                  .collection!.find({
                    'value.guildId': guild.id,
                  })
                  .toArray()

                const guildObject = await discordClient.guilds.cache.get(
                  guild.id
                )
                const member = await guildObject?.members.fetch(
                ...
                return {
                  ...
                  numLastVersionThreads: threads.filter(
                    thread =>
                      thread.value.version === ChatGPTConversation.latestVersion
                  ).length,
...
\`\`\``
    ...
    if (
      ...
      await this.sendReply(
        ...
        `${
          this.allowExternals
            ? 'Allowing anyone to converse in this thread.'
            : 'Not allowing anyone else to converse in this thread any more.'
        }

To toggle again, type \`<TOGGLE_EXTERNALS>\` in here again.`,
        messageToReplyTo
      )
    ...
    if (inputValue === '<DEBUG>') {
      const userOrServerHasPermissions =
        user.id === adminPingId || botConfig.promptPermissions.includes(user.id)

      if (userOrServerHasPermissions) {
        const { lastUpdated, nextSummaryMessageCount } = this
        const totalTokens = this.messageHistory
          .map(id => this.messageHistoryMap[id])
          .reduce((sum, item) => sum + getNumTokens(item), 0)
        const responseTokens: CreateCompletionResponseUsage =
          this.messageHistory
            .map(id => this.messageHistoryMap[id])
            .reduce(
              (sum, item) => {
...
\`\`\`
${fullPrompt.map(item =>
  JSON.stringify(item, null, '  ').replaceAll('```', '\\`\\`\\`')
)}
\`\`\`
===END PROMPT - TOKENS: ${numTokensFromMessages(fullPrompt)} ===`

      await new MultiMessage(channel, undefined, messageToReplyTo).update(
        prompt,
        true
      )
    ...
    if (inputValue.startsWith(historyPrefix) && userId === adminPingId) {
      const historyConfig = await db.get<HistoryConfig>(
        `HISTORY-CONFIG-${userId}`
      )

      if (!historyConfig) {
        await new MultiMessage(channel, undefined, messageToReplyTo).update(
          'Please ping the bot owner to allow you to use the <HISTORY> command.',
          true
        )
        return
      }
      ...
      const history = this.messageHistory
        .slice(this.messageHistory.length - toShow)
        .map(id => this.messageHistoryMap[id])
      const response = `History: \n${history
        .map(item => messageToPromptPart(item))
        .join('\n')}`
    ...
    try {
      const moderationResponse: AxiosResponse<CreateModerationResponse> =
      ...
      const failures = entries.filter(([key, value]) => {
        return (
          value >
          botConfig.moderationThreshold[
            key as keyof CreateModerationResponseResultsInnerCategoryScores
          ]
        )
      })

      logMessage(
        `${await this.getLinkableId()} Moderation:`,
        Object.fromEntries(
          entries.map(([key, value]) => [key, value.toFixed(2)])
        )
      )
...
${failures
  ...
  .join('\n')}]]`,
    ...
    if (messageToReplyTo) {
      ...
      await this.handleSpecialKeywords(
        openai!,
        inputValue,
        usingOpenAIForServer,
        userId,
    ...
    if (usingOpenAIForServer) {
      ...
      if (
        currentConfig.maxMessagePerUser !== -1 &&
        ...
        if (
          ...
          await channel.send({
            content: '',
            embeds: [
              new EmbedBuilder().setTitle('Message Limit')
                .setDescription(`<@${userId}> - You have sent ${
                messageCountForUser.limitCount
              } messages out of the maximum allowed ${
                currentConfig.maxMessagePerUser
              }.
                             
When you reach ${
                currentConfig.maxMessagePerUser
              }, you won't be able to send any more messages until an Admin allows it, or until you provide your own API key to me.
...
Thank you for your understanding.`),
    ...
    if (multi.messageList.length > 0) {
      this.lastDiscordMessageId =
        multi.messageList[multi.messageList.length - 1].message.id
      await this.persist()
    }
  }

  private async handleSpecialKeywords(
    openai: OpenAIApi,
    input: string,
    usingOpenAIForServer: boolean,
    userId: string,
    ...
    const imageDescriptions = await this.getImageDescriptions(
      input,
      usingOpenAIForServer,
      userId,
      currentConfig,
  ...
  private async trySummariseThread(
    openai: OpenAIApi,
    userId: string,
    channel: AnyThreadChannel
  ) {
    try {
      const allMessagesInHistory = this.messageHistory.map(
        id => this.messageHistoryMap[id]
      )

      const lastMessages = getLastMessagesUntilMaxTokens(
...
Source: ${lastMessages}`)
  ...
  private async getImageDescriptions(
    inputValue: string,
    usingOpenAIForServer: boolean,
    userId: string,
    currentConfig: ConfigForIdType,
    ...
    if (descriptions.length > 0) {
      const userHasExceptionRole = await this.userHasExceptionRole(
        userId,
        currentConfig
      )

      const userShouldBeRestricted =
        usingOpenAIForServer &&
      ...
      if (userShouldBeRestricted) {
        ...
        if (
          messageCountForUser.imageLimitCount >= currentConfig.maxImagePerUser
        ) {
          await channel.send({
            content: '',
            embeds: [
              new EmbedBuilder().setTitle('Image Limit').setDescription(`
...
Thank you for your understanding.`),
          ...
          descriptions = this.trimDescriptions(
            messageCounter,
            userId,
            currentConfig,
            descriptions
  ...
  private async userHasExceptionRole(
    userId: string,
    currentConfig: ConfigForIdType
  ) {
    const guild = await discordClient.guilds.fetch(this.guildId)
    ...
    return currentConfig.exceptionRoleIds.some(exceptionRoleId =>
      member.roles.cache.has(exceptionRoleId)
    )
  }
  ...
  private trimDescriptions(
    ...
    const remainingImages = Math.max(
      currentConfig.maxImagePerUser - messageCountForUser.imageLimitCount,
      0
    )
  ...
  public async deleteMessages(toDelete: number) {
    const deletedIndices = this.messageHistory.splice(
      this.messageHistory.length - toDelete
    )
    const pinecone = await getPineconeClient()
    if (pinecone != null) {
    ...
    if (pinecone != null && config.maxMessagesToEmbed > 0) {
      ...
      for (let i = 0; i < withoutEmbedding.length; i++) {
        ...
        if (!withoutEmbeddingElement.timestamp) {
          withoutEmbeddingElement.timestamp =
            this.lastUpdated - 1000 * (withoutEmbedding.length - i)
      ...
      if (messageToReplyTo && channel) {
        await this.sendReply(
          channel,
          `Preparing to embed ${firstN.length} messages...`,
          messageToReplyTo
        )
      } else {
        logMessage(
          `${await this.getLinkableId()}: Preparing to embed ${
            firstN.length
          } messages...`
        )
      }
      ...
      try {
        ...
        if (messageToReplyTo && channel) {
          await this.sendReply(
            channel,
            `Cannot create embeddings: ${printArg(e)}`,
            messageToReplyTo
          )
        } else {
          logMessage(
            `${await this.getLinkableId()}: Cannot create embeddings`,
            e
          )
        }
        return
      ...
      if (messageToReplyTo && channel) {
        await this.sendReply(
          channel,
          `Embeddings created for ${firstN.length} messages.`,
          messageToReplyTo
        )
      } else {
        logMessage(
          `${await this.getLinkableId()}: Embeddings created for ${
            firstN.length
          } messages.`
        )
      }
      ...
      try {
        ...
        if (messageToReplyTo && channel) {
          ...
          logMessage(
            `${await this.getLinkableId()}: Error while trying to save embeddings: ${printArg(
              e
            )}`
          )
        }
  ...
  private async testQuery(
    ...
    const messages = await this.getRelevantMessages(
      user,
      openai,
      orderWeight,
      vector
    ...
    const resultString = topK
      .map(item => {
        ...
        return `- ${indexString} S:${scoreString} O:${orderString} W:${weightString} ${getNumTokens(
          item.message
        )
          .toString()
          .padStart(4, '0')}
${messageToPromptPart(item.message)}`
  ...
  private getSummarisePrompt(
    lastMessages: MessageHistoryItem[]
  ): ChatCompletionRequestMessage[] {
...
${lastMessages.map(item => messageToPromptPart(item)).join('\n')}`,
  ...
  private async getFullPrompt(
    ...
    let availableTokens =
      modelInfo.MAX_ALLOWED_TOKENS - numInitialPromptTokens - inputTokens
    ...
    if (totalTokensFromHistory < availableTokens) {
      ...
      result.push(
        ...allMessagesInHistory.map((item): ChatCompletionRequestMessage => {
          switch (item.type) {
            case 'human':
              return {
                role: 'user',
    ...
    const tokensForRecentMessages = Math.min(
      config.maxTokensForRecentMessages,
      availableTokens
    )
    ...
    const unseenRelevantMessages = relevancyCheckCache.results.filter(
      item => !latestMessages.find(message => message.id === item.message.id)
    )

    const latestMessagesPlusNewMessage = latestMessages.concat(inputMessageItem)

    // get top N until MAX_ALLOWED_TOKENS is reached?
    const usedTokensFromMessages = latestMessagesPlusNewMessage.reduce(
      (sum, item) => sum + getNumTokens(item),
      0
    )
    ...
    includedRelevantMessages.sort((a, b) => a.match.index - b.match.index)
    ...
    if (debug) {
      ...
      relevantSuffix = `---END RELEVANT - TOKENS: ${includedRelevantMessages.reduce(
        (sum, item) => sum + getNumTokens(item.message),
        0
      )} ---`
    } else {
    ...
    if (includedRelevantMessages.length > 0) {
      ...
      result.push(
        ...includedRelevantMessages.map(
          (relevancy): ChatCompletionRequestMessage => {
    ...
    result.push(
      ...latestMessagesPlusNewMessage.map(
  ...
  private async getRelevantMessages(
    ...
    try {
      ...
      const sorted = queryResult.matches
        .map(match => {
          return {
      ...
      for (const match of sorted) {
        ...
        const matchingMessage =
          this.messageHistoryMap[this.messageHistory[match.index]]

        wantedMessages[index] = Math.max(
          wantedMessages[index] ?? 0,
          match.score
        )

        if (matchingMessage.type === 'human') {
          if (this.messageHistory.length > index + 1) {
            wantedMessages[index + 1] = Math.max(
              wantedMessages[index + 1] ?? 0,
              match.score
            )
          }
        } else {
      ...
      const entries = Object.entries(wantedMessages) as any as [
        index: number,
        score: number,
      ][]
      ...
      return entries.map(([index, score]): RelevancyResult => {
        const matchingMessage =
          this.messageHistoryMap[this.messageHistory[index]]
  ...
  public sendReply(
    ...
    return new MultiMessage(channel, undefined, messageToReplyTo).update(
      message,
      true
    )
  }
  ...
  private async getDebugName(user: User) {
    return this.isDirectMessage
      ? user.username
      : await getGuildName(this.guildId)
  }
      ...
      results.map(async result => {
        ...
        if (info != null) {
          try {
            ...
            logMessage(
              'Failed to initialise info with thread id ',
              info.threadId,
              e
            )
          }
