  ...
  public async initialise(): Promise<void> {
    ...
    if (this.isDirectMessage) {
      ...
      newMessages = newMessages.filter(
        message => message.author.id !== currentBotId
      )
...
I will respond to this message now.]]`,
      ...
      this.handlePrompt(
        ...
        logMessage('INITIALIZEThreads', 'failed to handle prompt...', e)
      )
    ...
    if (!threadResponse.success) {
      logMessage(
        `${await this.getLinkableId()}: Failed to get thread, status: ${
      ...
      if (threadResponse.status === 404) {
        logMessage(
          `Thread ${await this.getLinkableId()} deleted (or never existed)! Marking as deleted...`
        )
        this.deleted = true
        await this.persist()
    ...
    logMessage(
      `${await this.getLinkableId()} new messages: ${newMessages.length}.`
    )
    ...
    if (!thread.isThread()) {
      ...
      if (lastRelevantMessage != null) {
        logMessage(
          `Found message for ${await this.getLinkableId()}`,
...
I will respond to this message now.]]`,
          ...
          logMessage(
            `A new message is being handled for ${await this.getLinkableId()} already, no need to respond.`
          )
        }
      }
...
I will respond to your last prompt now.]]`,
      ...
      logMessage(
        `Found no messages from user for thread: ${await this.getLinkableId()}`
      )
    }
    this.lastDiscordMessageId = newMessages[newMessages.length - 1].id
    await this.persist()
