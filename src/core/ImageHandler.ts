...
class ImageHandlerRequest {
  ...
  async start(message: Message, isCommand: boolean = false) {
    ...
    if (isCommand) {
      replyPromise = message
        .edit({
          embeds: [
            message.embeds[0],
            new EmbedBuilder().setTitle('Image Prompt')
...
> ${this.storage.description}`),
    ...
    const response: AxiosResponse<ImagesResponse> =
    ...
    await reply.edit({
      files: [attachment],
      embeds: isCommand ? [message.embeds[0], finalEmbed] : [finalEmbed],
    })
  }
}
...
export class ImageHandler {
  ...
  constructor(
    ...
    public storage: ImageHandlerStorage
  ) {
    this.requests = storage.subItems.map(
      item => new ImageHandlerRequest(this, item)
    )
  }
