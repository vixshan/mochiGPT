...
async function getSplitImageData(
  url: string,
  maxHeight: number
): Promise<Buffer[]> {
  ...
  try {
    ...
    while (!finished) {
      ...
      newCanvas
        .getContext('2d')
        .putImageData(
          ctx.getImageData(0, currStart, image.width, currEnd - currStart),
          0,
          0
        )
  ...
  static async handle(
    ...
    const bufferList: (
      ...
      descriptions.map(async description => {
        try {
          const url = `https://api.wolframalpha.com/v1/simple?i=${encodeURIComponent(
            description
          )}&appid=${WOLFRAM_APP_ID}&width=1024&fontsize=38&units=metric&layout=labelbar`
      ...
      try {
        ...
        const embeds = [
          new EmbedBuilder()
            ...
            .setDescription(
              `Failed to get image for description ${description}: ${message}`
            )
            .setColor(0xff0000),
        ]
