...
export const retrieveConversation = async (
  ...
  const fromDb = await db.get<BaseConversation>(
    BaseConversation.getDBKey(threadId)
  )
  ...
  if (fromDb != null) {
    if ((fromDb as ChatGPTConversation).version !== undefined) {
      result = await ChatGPTConversation.handleRetrievalFromDB(
        fromDb as ChatGPTConversation
      )
    } else {
      result = null
    }
